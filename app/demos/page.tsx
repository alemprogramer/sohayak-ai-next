        ...
        <section className="mt-20">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center mb-16">
              {data?.map((item: any, i: any) => (
                <Card key={i} className="bg-white/80 backdrop-blur-sm border-0 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 w-full flex flex-col justify-between">
                  <CardHeader className="text-center pb-4">
                    ...
                    <CardTitle className="text-xl font-bold text-gray-900">{item?.name} (Premium)</CardTitle>
                    <CardDescription className="text-gray-600">
                      {item?.text || "Ultra-low latency real-time voice assistant demo"}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="text-center">
                    <a href={`/v2?agentId=${item._id}`} target="_blank" rel="noopener noreferrer" className="block w-full">
                      <Button className="bg-teal-600 hover:bg-teal-700 text-white w-full flex items-center justify-center gap-2 cursor-pointer">
                        <Mic className="w-4 h-4" />
                        Try Premium Demo
                      </Button>
